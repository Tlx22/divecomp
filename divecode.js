...
function buttons(){
 if(HAL.edge(2)){if(S.showLog)S.showLog=false;else if(S.mode=='SURFACE'&&!S.compLocked)S.mode='MENU';else if(S.mode=='MENU'){if(S.dirty)saveSet();S.mode='SURFACE';S.cur=0}else if(S.mode=='DIVE')S.view=(S.view+1)%4}
 if(HAL.edge(0)&&S.mode=='MENU')S.cur=(S.cur+1)%8;
...
function render(now){u8g2.clearBuffer();const F='6x10',fo=S.fo2,pp=(S.ppo2/100).toFixed(2),lk=Math.ceil(S.lock/3600);
 if(S.mode=='SURFACE'&&S.showLog){T(F,0,9,'DIVE LOG (last 6)');u8g2.drawHLine(0,11,128);const l=prefs.get('logs',[]).slice(-6).reverse();if(!l.length)T('5x7',0,22,'No dives logged yet');l.forEach((e,i)=>T('5x7',0,20+i*8,`${mmss(e[0])} ${e[1]}m O2${e[2]}% ${ALG[e[3]].slice(0,4)}${e[4]?' DECO':''}`));T('5x7',0,62,'MODE: back')}
 ...
 else{const W=[];if(S.compLocked)W.push('COMPUTER LOCKED');if(S.fault)W.push('SENSOR FAULT');if(S.wc)W.push('CEILING BREACH!');if(S.wp)W.push('HIGH PPO2!');if(S.wa)W.push('ASCENT TOO FAST!');if(S.texc)W.push('TABLE LIMIT EXCEEDED');if(S.wcns)W.push('CNS HIGH!');if(S.wndl2&&!S.deco)W.push('NDL 2 MIN');
  const crit=S.compLocked||S.wc||S.wp||S.wa||S.wcns;if(crit&&Math.floor(now/400)%2==0){u8g2.drawHLine(0,0,128);u8g2.drawHLine(0,63,128);u8g2.drawBox(0,0,2,64);u8g2.drawBox(126,0,2,64)}
  ...
  T('5x7',0,19,'DEPTH m');T('5x7',80,19,S.deco?'CEIL m':'NDL min');T('L22',0,41,S.depth.toFixed(1));vsi();
  ...
  if(S.view!=0){}
  else if(W.length)T('6x12',0,58,W[Math.floor(now/1500)%W.length]);
